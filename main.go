...
func main() {
	...
	go StartProcess("sleep")      // start a thread with sleep TODO: make real thread
	StartProcessWithFork("sleep") // fork a process TODO: make real process fork
...
func StartProcessWithFork(cmd string, args ...string) *exec.Cmd {
	command := exec.Command(cmd, args...)
	//syscall.ForkExec(syscall.SYS_FORK, 0, 0)
	if err := command.Start(); err != nil {
		panic("Failed to start detached process because of: " + err.Error())
	}
	return command
